...
const PsychiatristList = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5000/users/psychiatrists", {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("jwt-token")}`,
        },
      })
      .then((response) => {
        const filteredPsychiatrists = response.data.filter(
          (user) =>
  ...
  useEffect(() => {
    if (selectedPsychiatrist) {
      axios
        .get(
          `http://localhost:5000/users/appointments/psychiatrist/${selectedPsychiatrist._id}`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("jwt-token")}`,
            },
        ...
        .then((response) => {
          ...
          const formattedEvents = formatAvailabilitiesToEvents(
            selectedPsychiatrist.availability,
            response.data
          );
    ...
    availabilities.forEach((slot, slotIndex) => {
      ...
      for (let time = startTotalMinutes; time < endTotalMinutes; time += 30) {
        ...
        const matchingAppointment = appointments.find((apt) => {
          ...
          const [aptStartHour, aptStartMinute] = apt.startTime
            .split(":")
            .map(Number);
  ...
  const handleBookAppointment = async (eventInfo) => {
    ...
    try {
      ...
      // Refresh events
      const updatedEvents = formatAvailabilitiesToEvents(
        response.data.updatedAvailability,
          ...
          >
            ...
            <input
              ...
              style={{
                width: "60%",
                maxWidth: "400px",
                padding: "12px 20px 12px 40px", // Adjusted padding to accommodate icon
                fontSize: "16px",
                borderRadius: "25px",
                ...
                <SwiperSlide key={psychiatrist._id}>
                  ...
                  >
                    ...
                    <div className="psychiatrist-info">
                      <h4 style={{ margin: "10px 0", fontSize: "1.5rem" }}>
